  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <p className="text-sm font-semibold text-slate-800">내 연봉과 비슷한 기업</p>
        <div className="mt-2 flex flex-wrap gap-2">
          {similar.map((c) => (
            ...
            >
              {c.name} <span className="text-slate-400">{manwon(c.avgSalary)}</span>
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        <p className="font-semibold text-slate-800">
          기업별 평균연봉 <span className="text-xs font-normal text-slate-400">({filtered.length}개 표시 중)</span>
        </p>
