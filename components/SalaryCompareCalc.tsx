    ...
    <div className="space-y-6">
        ...
        <label className="block text-sm font-medium text-[#5B6478]">
          ...
          <div className="mt-1 flex items-center gap-1">
            <input
              ...
              className="w-full rounded-lg border border-[rgba(46,68,148,0.22)] bg-white px-3 py-2.5 text-right tabular-nums"
            />
      ...
      <div className="rounded-xl border border-[rgba(46,68,148,0.14)] bg-[rgba(46,68,148,0.06)] p-5">
        ...
        <p className="mt-2 text-xs text-[#8B93A6]">
          티어는 우리가 수집한 {stats.count}개 기업의 실제 연봉 분포를 4등분(사분위)한 기준입니다 — 임의로 정한
          등급이 아니라 데이터가 갱신될 때마다 자동으로 다시 계산됩니다.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <select
          ...
          className="rounded-lg border border-[rgba(46,68,148,0.22)] bg-white px-2 py-1.5 text-xs text-[#5B6478]"
        >
      ...
      <div className="rounded-xl border border-[rgba(46,68,148,0.14)] bg-white p-5">
        <p className="font-semibold text-[#1B2A4A]">
          기업별 평균연봉 <span className="text-xs font-normal text-[#8B93A6]">({filtered.length}개 표시 중)</span>
        </p>
            ...
            <div key={c.corpCode}>
              <div className="mb-0.5 flex justify-between text-xs">
                <span className="text-[#5B6478]">
                  {c.name} <span className={`ml-1 rounded px-1 text-[10px] ${TIER_COLOR[c.tier]}`}>{c.tier}</span>
                </span>
